...
parser.add_argument('--load_folder', type=str, default='./',
                       help='directory to load embedding matrices')
...
char_indices = np.load(args.load_folder+'char_indices.npy').item()
indices_char = np.load(args.load_folder+'indices_char.npy').item()
...
embedding_matrix = np.load(args.load_folder+'char_embedding_matrix.npy')
# ===============================================
# =           Loading word embeddings           =
# ===============================================

# with open(args.load_folder+'tokenizer.pickle', 'rb') as handle:
...
# word_embed_matrix = np.load(args.load_folder+'word_embedding_matrix.npy')
...
out1 = Dense(256, activation='relu')(char_att)
...
model = Model(inputs = sentence_input, outputs = out7)
...
model.fit(X_train, Y_train, batch_size=batch_size, epochs=epochs,  validation_data=[X_vali, V_test], verbose=1)
